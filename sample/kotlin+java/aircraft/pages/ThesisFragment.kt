...
class ThesisFragment : DJIFragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        return inflater.inflate(R.layout.frag_thesis, container, false)
    }
    ...
    private fun updateAvailableCamera(availableCameraList: List<ComponentIndexType>) {
        ...
        for (cameraIndex in availableCameraList) {
            ...
            ft.replace(frameLayout.id, ThesisDetailFragment.newInstance(cameraIndex, onlyOneCamera), cameraIndex.name)
        }
        ft.commitAllowingStateLoss()
    }
}
