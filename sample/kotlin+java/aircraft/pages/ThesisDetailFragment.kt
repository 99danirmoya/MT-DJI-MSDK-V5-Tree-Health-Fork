...
// Import necessary classes for ThingsBoard integration

class ThesisDetailFragment : DJIFragment() {
    ...
    companion object {
        ...
        fun newInstance(cameraIndex: ComponentIndexType, onlyOneCamera: Boolean): ThesisDetailFragment {
            val args = Bundle()
            args.putInt(KEY_CAMERA_INDEX, cameraIndex.value())
            args.putBoolean(KEY_ONLY_ONE_CAMERA, onlyOneCamera)
            val fragment = ThesisDetailFragment()
    ...
    // --- New UI elements for ThingsBoard integration ---
    private lateinit var btnPublishRandomThingsBoard: Button
    private lateinit var tvLastPublishedThingsBoardNumber: TextView
    private lateinit var btnCaptureAndSendImage: Button
    private lateinit var tvTreeIdDisplay: TextView
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        val layoutId: Int = if (onlyOneCamera) {
            R.layout.fragment_thesis_detail_single
        } else {
            R.layout.fragment_thesis_detail
        }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        btnPublishRandomThingsBoard = view.findViewById(R.id.btn_publish_random_thingsboard)
        tvLastPublishedThingsBoardNumber = view.findViewById(R.id.tv_last_published_thingsboard_number)
        btnCaptureAndSendImage = view.findViewById(R.id.btn_capture_and_send_image)
        tvTreeIdDisplay = view.findViewById(R.id.tv_tree_id_display)
        btnReport = view.findViewById(R.id.btn_report)
        // --- End of new UI elements initialization ---
        ...
        // Set listener for the new capture and send image button
        btnCaptureAndSendImage.setOnClickListener {
            // Call the ViewModel function to capture and send the image
            viewModel.captureAndSendImageToThingsBoard()
        }

        // --- Set listener for the "0/1" button ---
        btnPublishRandomThingsBoard.setOnClickListener {
            // This button now ONLY toggles the hasFinished state in the ViewModel
            viewModel.toggleHasFinishedState()
        }

        btnReport.setOnClickListener {
            // Create an Intent to start the new ThesisTreeReport activity
            val intent = Intent(activity, ThesisTreeReport::class.java)
            // Start the activity
            startActivity(intent)
        }

        // Initially disable the report button until hasFinishedState is 1
    ...
    private fun initViewModel() {
        ...
        viewModel.hasFinishedState.observe(viewLifecycleOwner) { hasFinishedValue ->
            tvLastPublishedThingsBoardNumber.text = "Last published hasFinished: $hasFinishedValue"
            // --- NEW LOGIC: Enable/Disable btnReport based on hasFinishedValue ---
            btnReport.isEnabled = (hasFinishedValue == 1)
        }

        viewModel.currentTreeId.observe(viewLifecycleOwner) { treeId ->
            tvTreeIdDisplay.text = "treeId: $treeId"
        }
    }
    ...
    private val cameraSurfaceCallback = object : SurfaceHolder.Callback {
        ...
        override fun surfaceChanged(holder: SurfaceHolder, format: Int, width: Int, height: Int) {
            this@ThesisDetailFragment.width = width
            this@ThesisDetailFragment.height = height
            updateCameraStream()
        }
